...
COUNCIL_TSX = '''import { useState } from 'react';
...
cp = os.path.join(root, 'src', 'routes', 'Council.tsx')
open(cp, 'w').write(COUNCIL_TSX)

# ============ 2. Patch Research.tsx (engine dropdown) ============

rp = os.path.join(root, 'src', 'routes', 'Research.tsx')
rs = open(rp).read()

old = "  const [question, setQuestion] = useState('');"
assert old in rs, 'Research question state anchor missing'
rs = rs.replace(old, old + "\n  const [tier, setTier] = useState<'free' | 'paid'>('free');", 1)

old = "supabase.functions.invoke('research', { body: { question: question.trim() } })"
assert old in rs, 'Research invoke anchor missing'
rs = rs.replace(old, "supabase.functions.invoke('research', { body: { question: question.trim(), tier } })", 1)
...
inserted = False
for ln in lines:
    if (not inserted) and ('<button onClick={run}' in ln):
        out.append(dropdown)
        inserted = True
    out.append(ln)
assert inserted, 'Research button anchor missing'
rs = '\n'.join(out)

open(rp, 'w').write(rs)

print('Update 5d patched: council size/tier dropdowns + research engine selector')
