...
# ============ 1. Create SelfBuild.tsx ============

SELFBUILD_TSX = '''import { useEffect, useState } from 'react';
...
sp = os.path.join(root, 'src', 'routes', 'SelfBuild.tsx')
os.makedirs(os.path.dirname(sp), exist_ok=True)
open(sp, 'w').write(SELFBUILD_TSX)

# ============ 2. Patch App.tsx (route + import) ============

ap = os.path.join(root, 'src', 'App.tsx')
a = open(ap).read()

old = "import VideoGen from './routes/VideoGen';"
...
# ============ 3. Patch Sidebar.tsx (founder-only nav link) ============

vp = os.path.join(root, 'src', 'components', 'Sidebar.tsx')
s = open(vp).read()

old = "import { useState } from 'react';"
assert old in s, 'Sidebar react import anchor missing'
s = s.replace(old, "import { useEffect, useState } from 'react';", 1)

old = "  const activeChat = location.pathname.startsWith('/chat/');"
...
print('Update 4g patched: SelfBuild route + founder-only sidebar link')
