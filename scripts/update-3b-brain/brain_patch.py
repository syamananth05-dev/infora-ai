...
# ---------- 1. New page: src/routes/Knowledge.tsx ----------
...
anchor1 = "import Settings from './routes/Settings';"
assert anchor1 in a, 'App.tsx import anchor missing'
a = a.replace(anchor1, anchor1 + "\nimport Knowledge from './routes/Knowledge';", 1)

anchor2 = '          <Route path="/settings" element={<Settings />} />'
assert anchor2 in a, 'App.tsx route anchor missing'
a = a.replace(anchor2, '          <Route path="/knowledge" element={<Knowledge />} />\n' + anchor2, 1)
open(ap, 'w').write(a)

# ---------- 3. Patch src/components/Sidebar.tsx ----------

sp = os.path.join(root, 'src', 'components', 'Sidebar.tsx')
s = open(sp).read()

anchor3 = '        <NavLink to="/projects" label="Projects" icon="📁" />'
assert anchor3 in s, 'Sidebar nav anchor missing'
s = s.replace(anchor3, anchor3 + '\n        <NavLink to="/knowledge" label="Knowledge" icon="🧠" />', 1)
open(sp, 'w').write(s)

print('Brain frontend patched: Knowledge.tsx + App.tsx route + Sidebar nav')
